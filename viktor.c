 ...
 *
 * -------------------------
 * This file contains methods: game_start, credits_screen, check_if_high_score, get_scores_from_file, load_default_scores, print_game_info,
	...
	while(1)
	{
		...
		else if(selection == 1)
		{
			...
			while(1)
			{
                ...
                shuffle_array(answers, q.a0);
                char c_answer = display_answers(answers, q.a0);

                printf("\nSelection: ");

                /*Get correct input*/
                char c_input = handle_input("1234EeFf");

                /*If fifty_fifty*/
                if (c_input == 'f' || c_input == 'F')
                {
                    fifty_fifty(answers, q.a0);
                	stats.streak /= 2;
                    clear_screen();
    				print_game_info();
    				print_player_stats(stats);
                    printf("%s\n\n",q.q);
                    display_answers(answers, q.a0);
                	printf("You picked 50/50. Your streak is halved!\n\nSelection: ");
                    c_input = handle_input("1234Ee");
                }
				...
				/*Lives == 0 -> Lose OR win*/
				if(stats.lives == 0 || stats.q_count == MAX_QUESTIONS)
				{
					clear_screen();
					print_player_stats(stats);
				...
				if(stats.q_count % 5 == 0 && stats.q_count != MAX_QUESTIONS)
				{
					stats.level++;
				}
			}
